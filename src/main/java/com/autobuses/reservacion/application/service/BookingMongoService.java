...
public class BookingMongoService implements BookingService<BookingDto> {
    ...
    public List<BookingDto> findAll() {
        List<BookingMongo> bookings = bookingRepository.findAll(); // Obtener todas las reservas desde MongoDB
    ...
    public BookingDto findById(Long id) {
        BookingMongo booking = bookingRepository.findById(id.toString()).orElse(null); // Buscar una reserva por su ID en MongoDB
        return booking != null ? bookingMapperMongo.toDto(booking) : null; // Mapear la reserva a un DTO
    }

    @Override
    public BookingDto create(BookingDto bookingDto) {
        BookingMongo booking = bookingMapperMongo.toEntity(bookingDto); // Convertir el DTO a una entidad
        BookingMongo savedBooking = bookingRepository.save(booking); // Guardar la reserva en MongoDB
    ...
    public BookingDto update(BookingDto bookingDto) {
        BookingMongo booking = bookingMapperMongo.toEntity(bookingDto); // Convertir el DTO a una entidad
        BookingMongo updatedBooking = bookingRepository.save(booking); // Actualizar la reserva en MongoDB
